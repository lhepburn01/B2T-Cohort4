let movies = [
	...
	{title:'Pulp Fiction', director:'Quetin Tarantino', year: 1994, status: 'No', cover:'images/pulpFiction.jpg'},
...
function displayMovies(){
	...
	for(let i = 0; i < movies.length; i++){
		document.getElementById('display').innerHTML += `<h1>${movies[i].title} directed by ${movies[i].director} </h1>
			<h1>${movies[i].year} Shown Status: ${movies[i].status} </h1> <img src=${movies[i].cover} />
